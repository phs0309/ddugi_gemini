...
const App = () => {
  ...
  useEffect(() => {
    setMessages([
      { role: 'model', text: '뚜기다. 부산 맛집 알려주는 일 한다.\n\n뭐 찾나? "해운대 국밥" 이런 식으로 말해.', restaurants: [] }
    ]);
  }, []);
...
대화할 때는 줄바꿈(\\n)을 적절히 사용해서 읽기 쉽게 해줘. 긴 설명보다는 핵심만 짧게 말해줘.
...
일반적인 대화와 함께 이 JSON 형식의 데이터를 제공해줘. 예를 들어, "이정도면 괜찮은 곳들이야." 또는 "여기 가봐." 같은 간결한 문장 뒤에 JSON 데이터를 포함시켜줘.
...
JSON 데이터는 항상 \`\`\`json ... \`\`\` 코드 블록 안에 넣어서 보내줘.`;
      ...
      const errorMessage = { role: 'model', text: '지금 바쁘다.\n\n나중에 다시 말해.', restaurants: [] };
